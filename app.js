...
const win = (userChoice, computerChoice) => {
  ...
  resultDiv.innerHTML = `${convertToUp(userChoice)} ${
    randomWin[randomNumber]
  } ${convertToUp(computerChoice)}. You win! ${winEmojis[randomNumberEmoji]}`;

  document.getElementById(userChoice).classList.add("win-border");
  setTimeout(
    () => document.getElementById(userChoice).classList.remove("win-border"),
    600
  );
};
...
const lose = (userChoice, computerChoice) => {
  ...
  resultDiv.innerHTML = `${convertToUp(computerChoice)} ${
    randomWin[randomNumber]
  } ${convertToUp(userChoice)}. You lose! ${loseEmojis[randomNumberEmoji]}`;

  document.getElementById(userChoice).classList.add("lose-border");
  setTimeout(
    () => document.getElementById(userChoice).classList.remove("lose-border"),
    600
  );
};
...
const tie = (userChoice, computerChoice) => {
  const tieEmojis = ["🤯", "😱", "🙈", "🧐", "🙀", "🙃"];
  const randomNumberEmoji = Math.floor(Math.random() * 6);
  resultDiv.innerHTML = `${convertToUp(computerChoice)} matches ${convertToUp(
    userChoice
  )}. It's a tie! ${tieEmojis[randomNumberEmoji]}`;

  document.getElementById(userChoice).classList.add("tie-border");
  setTimeout(
    () => document.getElementById(userChoice).classList.remove("tie-border"),
    600
  );
};
